...
| Auth             | JWT (PyJWT)                               |
| Testing          | pytest + pytest-asyncio + testcontainers + Hypothesis (property-based) |
...
See [README.md § Getting Started](README.md#getting-started) for setup steps.
...
| Tier | Tool | Scope |
|---|---|---|
| Integration | pytest + testcontainers | Full stack against a real PostgreSQL instance — no mocks |
| Property-based | Hypothesis | Mathematical invariants verified against hundreds of generated inputs |

### Why property-based testing in a fintech project?

Example-based tests verify behavior against hand-picked inputs — a useful baseline, but bounded by what the developer thought to test. In financial systems, the inputs that break invariants are often edge cases no one anticipated.

[Hypothesis](https://hypothesis.readthedocs.io/) generates hundreds of random inputs automatically and shrinks any failure to the minimal reproducing case. The investment is small; the coverage of the unexpected is large.

### Current Hypothesis coverage

**`tests/test_balance_invariant_hypothesis.py`** — DB-backed, `max_examples=20`

Verifies the double-entry balance invariant across variable-length entry sets (1–3 debits, 1–3 credits) generated by the `_amounts_summing_to` cut-point strategy. Also serves as a regression guard for the service-layer balance check.

**`tests/test_balance_invariant_hypothesis.py`** — Pure-function section, `max_examples=200`

Two property tests for `_convert_amount_usd`:

- `test_convert_amount_usd_returns_non_negative_int` — verifies return type and non-negativity across the full realistic rate range (0.0001–100.0, 6 decimal places)
- `test_fx_rounding_error_bounded_by_entry_count` — verifies that per-entry `ROUND_HALF_UP` accumulates at most N cents of USD imbalance across N entries (the mathematical upper bound for TD-050)

Two companion example-based tests in `tests/test_fx_rounding_unit.py` provide the existence proof — concrete inputs where the rounding error is exactly +1 or −1.

### Future expansion candidates

| Area | Property to verify |
|---|---|
| Idempotency | Same key + same body always returns the same response; different body always returns 422 |
| Void / reversal | Reversal entries are the exact mirror of original entries (amounts + directions) |
| Multi-currency | FX conversion is monotone: higher rate → higher converted amount |
