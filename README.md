...
Transactions are never updated or deleted. The status field models a `PENDING → POSTED → VOIDED` lifecycle defined in [ADR-005](docs/adr/005-transaction-status-lifecycle.md). Voiding a transaction creates a paired reversal transaction with opposite entry signs via `POST /api/v1/transactions/{id}/void`; the original transaction remains in the ledger for audit purposes. → [ADR-005](docs/adr/005-transaction-status-lifecycle.md)
