...
| TD-051 | Ledger correctness | `void_transaction` (`app/services/transaction_service.py`) sets the original transaction's `status = VOIDED` and posts a sign-flipped reversal transaction (`status = POSTED`). `calculate_balance` (`app/repositories/account_repository.py`) filters `WHERE status = 'POSTED'`, so the original is excluded from balance while the reversal is included — the two do not net to zero as a real 逆仕訳 would. Net effect on every affected account after a void: `-original`, not `0`. No existing test asserts the post-void net balance (`test_balance_excludes_voided_transaction` only checks the original is excluded, not that void nets to baseline). **Fix**: change the balance filter to include `VOIDED` (e.g. `status != PENDING`) so the original stays balance-effective and the reversal cancels it out arithmetically. Update `test_balance_excludes_voided_transaction` accordingly and add `test_balance_after_void_nets_to_zero`. Also reword ADR-005's "never UPDATE-d" rationale line to "amounts and entries are immutable; `status` is the single controlled mutable field (a state machine, not an edit)" to match the `original.status = TransactionStatus.VOIDED` assignment. | High |
| TD-052 | Performance | `GET /accounts/{id}/balance` (`app/api/v1/routes/accounts.py`) caches under key `balance:{id}:{as_of.date()}` but applies the same fixed `settings.balance_cache_ttl_seconds` TTL regardless of whether `as_of` is a closed historical date (arithmetically immutable) or today (still mutable). A longer/indefinite TTL for `as_of < today` would be safe and would reduce redundant recomputation. | Low |
